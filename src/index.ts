...
for await (const sourceFile of sourceFiles) {
  ...
  const clonedFilePath = path.join(args.clone, sourceFile);

  await $`mkdir -p ${dirname(clonedFilePath)}`;
  await $`cp ${sourceFilePath} ${clonedFilePath}`;
  await compressInPlace(clonedFilePath);
}

console.log("Generating a manifest file...");

const clonedFiles = new Glob("*/**").scan(args.clone);

for await (const clonedFile of clonedFiles) {
